...
export const test = async (req: Request, res: Response)=>{
    res.send('This data test is working!')
}
...
export const registerUser = async (req: Request, res: Response) =>{
    try{
        ...
        if(!password || password.length < 4){
            return res.status(400).send('Please provide password with minimum 4 characters.')
        }
...
export const getProfile = async (req: Request, res: Response)=>{
    try{
        ...
        if(token){
            ...
            jwt.verify(formatToken, process.env.JWT_SECRET || '', {}, (err, response)=>{
                if(err) throw err;
